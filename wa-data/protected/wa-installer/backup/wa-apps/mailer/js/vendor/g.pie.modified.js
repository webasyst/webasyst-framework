...
(function () {
    ...
    function Piechart(paper, cx, cy, r, values, opts) {
        ...
        function sector(cx, cy, r, startAngle, endAngle, fill) {
            ...
            if (Math.abs(x1-x2)<1 && Math.abs(y1-y2)<1 && Math.abs(startAngle-endAngle) > 359) {
                // Full circle
                x2 = x1 + (cx - x1)*2;
                y2 = y1 + (cy - y1)*2;
                res = [
                        ...
                        "A", r, r, 0, +(Math.abs(endAngle - startAngle) > 180), 1, x1, y1,
                        "z"
                    ];
            } else if (x1 == x2 && y1 == y2) {
                // Empty arc
        ...
        var legend = function (labels, otherslabel, mark, dir) {
            ...
            for (var i = 0; i < len; i++) {
                ...
                bb = txt.getBBox(1);
                h += bb.height * 1.2;
                total_height += bb.height * 1.2;
                total_width = Math.max(total_width, bb.width);
            }
            ...
            var bb = chart.labels.getBBox(),
                tr = {
                    east: [0, -total_height / 2],
                    west: [-total_width - 2 * r - 20, -total_height / 2],
                    north: [-r - total_width / 2, -r - total_height - 10],
                    south: [-r - total_width / 2, r + 10]
                }[dir];
