...
// old files
$files = array(
    'photosPages.action.php',
    'photosPagesDelete.controller.php',
    'photosPagesSave.controller.php',
    'photosPagesSort.controller.php',
    'photosPagesUploadimage.controller.php',
);
...
if (file_exists($routing_path)) {
    ...
    foreach ($routing as $domain => $domain_routes) {
        foreach ($domain_routes as $route_id => $route) {
            if (isset($route['app']) && $route['app'] == 'photos') {
                $data = array('domain' => $domain, 'route' => $route['url']);
                $sql = "UPDATE photos_page SET domain = s:domain, route = s:route WHERE domain IS NULL";
    ...
    foreach ($routing as $domain => $domain_routes) {
        foreach ($domain_routes as $route_id => $route) {
            if (isset($route['app']) && $route['app'] == 'photos' && isset($route['_exclude'])) {
