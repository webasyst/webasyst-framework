...
(function ($) {
    ...
    $.widget('wa.fileupload', $.blueimp.fileupload, {
        ...
        options: {
                    ...
                    data.context.each(function (index) {
                        ...
                        that._transitionCallback(
                            ...
                            function (node) {
                                ...
                                if (!file.error) {
                                    setTimeout(function () {
                                        template.hide(200, function() {
                                            var not_hidden_lis = that._files.find('li:not(:hidden)');
                                            if (!not_hidden_lis.length) {
                                                $this.find('.upload-progress-status').hide();
                                                $this.find('.image-upload-dropbox').show();
                                            }
                                            if (!not_hidden_lis.filter(':not(.error)').length) {
                                                $('#upload-finished').show().html(
                                                    $_('Upload complete! <a href="#">Reload page</a> to view your photos.').replace('#', location.href)
                                                );
                                                $('#upload-invitation').hide();
                                                $('#upload-imagelist').hide();
                                            }
                                        });
                                    }, 5000);
                                }
            ...
            start: function () {
                var $this = $(this);
                $this.find('.upload-progress-content').show();
                $this.find('.fileupload-progressbar').css('width', '0%');
                $this.find('.upload-progress-status').show();
                $this.find('.image-upload-dropbox').hide();
                $('#upload-finished').hide();
                $this.data('fileupload')._files.children('.error').remove();
                $("#upload-error").hide();
        ...
        _renderDownload: function (files) {
            ...
            for (var i = 0; i < files.length; i++) {
                if (files[i].error) {
                    $("#upload-error").show();
                    that.find('.upload-progress-status').hide();
                    break;
                }
            }
