...
c&&c(d)}else c&&c(d)}})}},"json")},deleteAllAlbums:function(b,c,d){if(!b.length)return d&&d();var e=b.shift();a.photos.deleteAlbum(e,c,function(){a.photos.deleteAllAlbums(b,c,d)})},deleteAlbum:function(b,c,d){c?a.photos._deleteAlbumWithPhotos(b,d):a.post("?module=album&action=delete",{album_id:b},function(c){"ok"==c.status&&(a.photos.onDeleteAlbum(b),a.photos.goToHash(""),"function"==typeof d&&d())},"json")},_deleteAlbumWithPhotos:function(b,c){a.post("?module=photo&action=resolution",{album_id:b},
function(d){if("ok"==d.status){for(var d=d.data.photo_id,e=[],i=0,n=d.length;i<n;++i)e.push({name:"photo_id[]",value:d[i]});e.length?a.photos.massPost({url:"?module=photo&action=delete",photo_id:e,dataType:"json",fullSuccess:function(d){"ok"==d.status?a.photos.deleteAlbum(b,0,c):console&&console.log("Error while delete album")}}):a.photos.deleteAlbum(b,0,c)}},"json")},serializeData:function(b,c){if("object"===typeof b&&!a.isArray(b)&&"undefined"===typeof c){var d=[];for(c in b)b.hasOwnProperty(c)&&
d.push({name:c,value:b[c]});return d}a.isArray(b)||(b=[b]);for(var d=0,e=b.length,i=b[0];d<e;i=b[++d])"object"!=typeof i&&(i={name:c,value:i}),b[d]=i;return b},saveAccess:function(b){function c(b){for(var d=[],g=[],k=[],m=0,r=b.length;m<r;++m)d.push({name:"photo_id[]",value:b[m]});a.photos.massPost({photo_id:d,url:"?module=photo&action=manageAccessSave",dataType:"json",data:e,success:function(a,b){if("ok"==a.status){g=g.concat(a.data.denied_photo_id);for(var c=0,d=g.length;c<d;++c)b.push({name:"denied_photo_id[]",
value:g[c]});k=a.data.allowed_photo_id;c=0;for(d=k.length;c<d;++c)b.push({name:"allowed_photo_id[]",value:k[c]})}},fullSuccess:function(a){"ok"==a.status&&a.data.alert_msg&&onDeniedExist(a.data.alert_msg);"function"===typeof i&&i(a,k)}})}var d=b.photo_id||[],e=b.data||[],i=b.fn;onDeniedExist=b.onDeniedExist||function(a){alert(a)};resolution="boolean"===typeof b.resolution?b.resolution:!0;"object"===typeof e&&!a.isArray(e)&&(e=a.photos.serializeData(e));e.push({name:"photos_length",value:d.length});
resolution?a.post("?module=photo&action=resolution",a.isArray(d)?d:{photo_id:d},function(a){"ok"==a.status&&c(a.data.photo_id)},"json"):c(a.isArray(d)?d:[d])},addToAlbums:function(b){var c=b.photo_id||[],d=b.album_id||[],e=void 0==b.copy?1:b.copy,i=b.fn,n=b.onDeniedExist||function(a){alert(a)},c=a.photos.serializeData(c,"photo_id[]"),d=a.photos.serializeData(d,"album_id[]"),b=c.concat(d);b.push({name:"copy",value:+e});a.post("?module=photo&action=addToAlbums",b,function(b){if("ok"==b.status){for(var c=
[].concat(b.data.albums||[],b.data.old_albums||[]),d=0,g=c.length;d<g;++d){var e=c[d];a("#album-list li[rel="+e.id+"]").find(".count:first").text(e.count).end().find(".count-new:first").text(0<e.count_new?"+"+e.count_new:"")}b.data.alert_msg&&n(b.data.alert_msg)}"function"==typeof i&&i(b)},"json")},assignTags:function(b){var c=b.photo_id||[],d=b.tags||[],e=b.delete_tags||[],i=b.fn,n=b.onDeniedExist||function(a){alert(a)},b=[],b=a.isArray(c)?c:b.concat([{name:"photo_id[]",value:c},{name:"one_photo",
value:1}]);!a.isArray(d)||!d.length?(b.push({name:"tags",value:d}),e.length&&(b=b.concat(e))):b=b.concat(d);a.post("?module=photo&action=assignTags",b,function(b){if(b.status=="ok"){var c=b.data.cloud,d=a("#tag-cloud-block");if(a.isArray(c)&&!c.length||!c)d.hide();else{a("#tag-cloud").html(tmpl("template-tag-cloud",{cloud:c}));d.show()}var c=b.data.tags||{},e;for(e in c)if(c.hasOwnProperty(e)){d=c[e];a.photos._updateStreamCache(e,{tags:d})}a("#album-create-dialog").remove();b.data.alert_msg&&n(b.data.alert_msg)}typeof i==
"function"&&i(b)},"json")},makeStack:function(b){var c=b.shift();a.post("?module=photo&action=resolution",b,function(d){if("ok"==d.status){for(var d=d.data.photo_id,e=[],i=[],n=0,l=d.length;n<l;++n)d[n]!=c.value&&e.push({name:"photo_id[]",value:d[n]});a.photos.massPost({photo_id:e,data:{parent_id:c.value,photos_length:b.length},url:"?module=stack&action=make",dataType:"json",success:function(a,b){if("ok"==a.status&&"ok"==a.status){i=i.concat(a.data.denied_photo_ids);for(var c=0,d=i.length;c<d;++c)b.push({name:"denied_photo_id[]",
value:i[c]})}},fullSuccess:function(d){if("ok"==d.status){d.data.alert_msg&&alert(d.data.alert_msg);for(var e=0,i=b.length;e<i;++e)a.photos.photo_stream_cache.deleteById(b[e].value);a.photos.photo_stream_cache.updateById(d.data.parent_id,d.data.photo);a("#photo-list li.selected").trigger("select",!1);a.photos.makeStackAnimation([c].concat(b))}}})}},"json")},makeStackAnimation:function(b,c){var d=b[0].value,e=a('li[data-photo-id="'+d+'"]'),i=e.offset(),n=a(window);(i.top<n.scrollTop()||i.top>n.scrollTop()+
n.height())&&a("html, body").animate({scrollTop:i.top},300);for(var n=[],l=1;l<b.length;l++){var q=b[l].value,p=a('li[data-photo-id="'+q+'"]'),t=p.offset(),t=p.clone().css({"z-index":10,position:"absolute",top:t.top,left:t.left}).insertAfter(p);p.css({opacity:0});n.push(p.hide(300).promise());a.photos.photo_stream_cache.deleteById(q);n.push(t.animate({top:i.top,left:i.left},300).promise().done(function(){a(this).remove()}))}a.when.apply(a,n).done(function(){var b=e.hasClass("last");e.replaceWith(tmpl(a.photos.list_template,
{photos:[a.photos.photo_stream_cache.getById(d)],hash:a.photos.hash,last_login_time:a.photos.options.last_login_time,options:{}}));b||a('li[data-photo-id="'+d+'"]').removeClass("last");a('li[data-photo-id="'+d+'"]').addClass("highlighted");"function"===typeof c&&c()})},makeDeleteAnimation:function(b,c){var d={};a.each(b,function(a,b){d[b]=!0});var e=a("#photo-list").children().filter(function(){return!!d[a(this).data("photo-id")]}).animate({width:0},function(){e.remove();c&&c()})},highlightSidebarItem:function(){var b=
a.photos.hash;a("#p-sidebar li.selected").removeClass("selected");var c=a('#p-sidebar li a[href="#'+(b||"/")+'"]');c.length||(c=a('#p-sidebar li a[href^="#'+(b||"/")+'"]'));c.length&&c.parents("li:first").addClass("selected")},toggleFullScreen:function(){a.storage.get("photos/maximize_photo")?a("#wa-app").addClass("p-full-screen"):a("#wa-app").removeClass("p-full-screen")},hotkey_manager:{set:function(g){"undefined"===typeof g?a(document).bind("keydown",b).bind("keyup",c).bind("keydown",d).bind("keyup",
e):"rate"===g&&a(document).bind("keydown",d).bind("keyup",e)},unset:function(){a(document).unbind("keydown",b).unbind("keyup",c).unbind("keyup",d).unbind("keyup",e)}},getHashByPhotoId:function(b){return a.photos.hash+"/photo/"+b},setLazyLoad:function(){var b=null;a(window).lazyLoad({container:"#photo-list",load:function(){b=b||a("#photo-list li").length;a(window).lazyLoad("sleep");a(".lazyloading-wrapper .lazyloading-progress").show();a(".lazyloading-wrapper .lazyloading-link").hide();a.post("?module=photo&action=loadList",
{offset:b,hash:a.photos.hash},function(c){if(c.data.hash==a.photos.hash){var d=a("#photo-list");if(c.data.photos.length){var e=a.photos.photo_stream_cache.length();a.photos.photo_stream_cache.append(c.data.photos);d.find("li.last").removeClass("last");a.photos.renderPhotoListChunk(d,e,{string:c.data.string},function(){!a.photos.total_count||a.photos.total_count>a.photos.photo_stream_cache.length()?a(window).lazyLoad("wake"):(a(".lazyloading-wrapper .lazyloading-progress").hide(),a(".lazyloading-wrapper .lazyloading-link").hide(),
a(window).lazyLoad("stop"))});b+=c.data.photos.length}else a(".lazyloading-wrapper .lazyloading-progress").hide(),a(".lazyloading-wrapper .lazyloading-link").hide(),a(window).lazyLoad("stop")}},"json")}});a(".lazyloading-wrapper a.lazyloading-link").die("click.lazyloading").live("click.lazyloading",function(){a(window).lazyLoad("force");return!1})},unsetLazyLoad:function(){a(window).lazyLoad&&a(window).lazyLoad("stop");a(".lazyloading-wrapper a.lazyloading-link").die("click.lazyloading")},getAlbum:function(){return this.album=
this.album||null},setAlbum:function(a){this.album=a},unsetAlbum:function(){this.album=null},getPhotoId:function(){var a=/photo\/(\d+)[\/]*$/.exec(location.href);return a?parseInt(a[1]):null},isCurrentPhotoStack:function(){var a=this.getPhotoId();return!a?!1:(a=this.photo_stream_cache.getById(a))&&0<a.stack_count},goToHash:function(b,c){c="undefined"===typeof c?!0:c;b=b.replace(/^\/*/,"").replace(/\/*\s*$/,"");location.hash.replace(/^[^#]*#\/*/,"").replace(/\/*\s*$/,"")==b&&c?(a.photos.ignore_scrolltop=
!0,a.photos.dispatch()):location.hash=b?"/"+b+"/":"/"},goToAnchor:function(b){b=a("a[name="+b+"]");b.length&&a(document).scrollTop(b.position().top)},photo_stream_cache:new PhotoStream,onCreateAlbum:function(b,c){var d=tmpl("template-album-list-item",b),e=a("#album-list"),i;if(c){var n=e.find("li[rel="+c+"]");i=n.find("ul:first");i.length||(n.append('<ul class="menu-v with-icons"></ul>'),i=n.find("ul:first"),n.find(".count:first").after('<i class="icon16 darr overhanging collapse-handler" id="album-'+
c+'-handler"></i>'));i.prepend(d)}else i=e.find("ul:first"),i.length||(e.find(".p-empty-album-list").hide(),e.prepend('<ul class="menu-v with-icons"><li class="drag-newposition"></li></ul>').find("ul:first")),e.find("ul:first").prepend(d);e.find(".new-item").removeClass("new-item").mouseover();b.type||(d=a("#p-upload-step2 select[name=album_id]"),c?(d=d.find("option[value="+c+"]"),e=d.text().replace(/[^-]/g,"")+"-",d.after('<option value="'+b.id+'">'+e+" "+b.name+"</option>")):d.find("optgroup").prepend('<option value="'+
b.id+'">'+b.name+"</option>"))},onDeleteAlbum:function(b){var c=a("#album-list"),d=c.find("li[rel="+b+"]"),e=d.find("ul:first"),i=d.parents("ul:first"),n;d.hide();e.length?(d.prev(".drag-newposition:first").remove(),d.next(".drag-newposition:first").remove(),n=e.children("li"),n.hide().insertAfter(d),e.remove(),d.remove(),n.show()):d.next(".drag-newposition:first").remove().end().remove();i.length&&!i.find("li:not(.drag-newposition)").length&&(d=i.parent("li"),d.length&&d.find("i.collapse-handler").remove(),
i.remove());c.find("ul:first").length||c.find(".p-empty-album-list").show();a("#p-upload-step2 select[name=album_id]").find("option[value="+b+"]").remove()},fixRightToolbar:function(){a.photos.fixRightToolbar._handler=function k(){var b=a("#p-toolbar");b.length&&(b.hasClass("rendered")?b.removeClass("p-fixed"):(k.top||(k.top=b.children(":first").position().top),a(this).scrollTop()>k.top?b.addClass("p-fixed"):b.removeClass("p-fixed")))};a.photos.fixRightToolbar._handler.call(document);a(document).bind("scroll",
...
b+"]"),b.length?b.attr("selected",!0):c.find("option:first").attr("selected",!0))},massPost:function(b){function c(k){l?(q?(e=q.splice(0,d),b.data=i.concat(e)):b.data=i,l-=d,l=0<l?l:0,a.ajax(b)):"function"==typeof b.fullSuccess&&b.fullSuccess(k)}var d=b.chunk_count||25,e,i=[];if(a.isArray(b.data))i=i.concat(b.data);else if("object"==typeof b.data)for(var n in b.data)i.push({name:n,value:b.data[n]});b.type="post";var l=parseInt(b.count)||0,q=b.photo_id;if(b.photo_id){q=b.photo_id;if(a.isArray(q))q=
[].concat(q);else{i.push({name:"photo_id",value:q});b.data=i;b.success=b.fullSuccess;a.ajax(b);return}l=q.length}if("function"==typeof b.success){var p=b.success;b.success=function(a){p(a,i);c(a)}}else b.success=c;c()},confirmDialog:function(b){var c=a("#confirm-dialog");c.length||(c=a('<div id="confirm-dialog"></div>'),a("body").append(c));b.url&&c.load(b.url,function(){var d=a.extend({},b);delete d.url;var e=a(this),i=d.onLoad;d.onLoad=function(){c.find("input[type=submit]").focus();typeof i===
"function"&&i()};e.find("div:first").waDialog(d)})},showServerError:function(a){a?(console&&console.log("Server error occurred:\n"+a),alert(a)):console&&console.log("Server error occurred")},initAlbumThumbsDragAndDrop:function(b,c){b.sortable({handle:"img",distance:5,tolerance:"pointer",stop:function(b,d){a.post("?module=album&action=move",{id:d.item.data("album-id"),before_id:d.item.next().data("album-id")||0,parent_id:c})}})},hooks_manager:{handlers:{},bind:function(a,b,c){if(1==arguments.length&&
