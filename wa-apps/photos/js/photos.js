(function ($) {
    ...
    $.photos = {
                ...
                if (hash[0]) {
                    ...
                    for (var i = 0; i < hash.length; i++) {
                        ...
                        if (i < 2) {
                            if (i === 0) {
                                actionName = h;
                            } else if (actionName == 'tag' || actionName == 'search' || actionName == 'plugins' || actionName == 'pages') {
                                attrMarker = i;
                                break;
        ...
        pluginsAction: function(id) {
            $.photos.initClearance();
            var f = function() {
                if ($("#plugin-list li").length > 1) {
                    id = id || $.storage.get('photos/plugin_id') || $("#plugin-list li:first").attr('id').replace(/^plugin-/, ''),
                        plugin_li = $("#plugin-" + id);
                    if (plugin_li.length) {
                        $.storage.set('photos/plugin_id', id);
                        $("#plugin-list li.selected").removeClass('selected');
                        $("#plugin-" + id).addClass('selected');
                    } else {
                        if ($("#plugin-list li[id^=plugin-]").length) {
                            id = $("#plugin-list li.selected").attr('id').replace('plugin-', '');
                            $.storage.set('photos/plugin_id', id);
                        } else {
                            id = null;
                        }
                ...
                $.photos.loadPluginSettings(id, $.photos.onLoadPlugins);
            };
            if ($("#photos-plugins").length) {
                f();
            } else {
                $.photos.load("?module=plugins", f);
            }
        },

        loadPluginSettings: function(id, fn) {
            $.photos.initClearance();
            if (id) {
                var url = $("#plugin-" + id).attr('data-settings') ? '?plugin=' + id + '&module=settings' : '?module=plugins&id=' + id;
                $("#photos-plugins-content").load(url, function() {
                    fn(id);
                });
            } else {
                $('#photos-plugins-content span').html('');
            }
        },
        ...
        renderPhotoList: function() {
            ...
            target.empty();
            var chunk = 10;
            $.photos.renderPhotoListChunk(target, 0, chunk);
            if ($.photos.photo_stream_cache.length()) {
                $.photos.setLazyLoad();
            }
        },
        renderPhotoListChunk: function(target, offset, chunk, options, callback) {
            ...
            if(offset < length) {
                setTimeout(function(){
                    $.photos.renderPhotoListChunk(target, offset, chunk, options, callback);
                }, 100);
            } else{
        ...
        onLoadPhotoList: function() {
            ...
            if (album) {
                ...
                $('#album-list li[rel=' + album.id + ']').find('.count:first').text(album.count);
            }
            if(!view) {
                view = 'thumbs';
            }
        ...
        updateViewChildPhoto: function(data)
        {
            ...
            replaceImg(
                ...
                function() {
                    $.photos.afterRenderImg(this, photo);
                }
            );
        ...
        updateThumbRate: function(li, rate) {
            ...
            if (!rate) {
                ...
                }).removeClass('star').addClass('star-empty');
        ...
        initPhotoWidgets: function(data)
        {
            ...
            stream.hash = hash;
            stream.set(photo_stream.photos).setCurrent(stream.getById(photo_stream.parent_id));
        ...
        afterRenderImg: function(img, photo) {
            img.addClass('ui-draggable');
            if($.photos.widget) {
                $.photos.widget.loupe.init(photo);
            }
        },
        ...
        saveFields: function(data,success, fail) {
            ...
            $.post('?module=' + type + '&action=saveFields', {'data':data},
                function(response) {
                    if (response.status == 'ok') {
                        for(i in response.data.update) {
                            id = response.data.update[i]['id'];
                            if (!$.isArray(id)) {
                                id = [id];
                            }
                            info = response.data.update[i]['data'];
                            for (var i = 0, n = id.length; i < n; ++i) {
                                updated_info = $.photos.photo_stream_cache.updateById(id[i], info);
                            } 
                        }
                    }
                   //callback
                ...
                function(r) {
                    if (r.status == 'ok') {
                        var photo = r.data.photo,
                            salt = '?' + Date.parseISO(photo.edit_datetime),
                            proper_thumb = $.photos._chooseProperThumb(photo);
                ...
                function(r) {
                    if (r.status == 'ok') {
                        ...
                        $.photos.massPost({
                            ...
                            dataType: 'json',
                            data: { photos_length: original_photo_id.length },
                            ...
                            fullSuccess: function(r) {
                                ...
                                if (r.status == 'ok' && !r.data.denied_all) {
                                    $.photos.goToHash(hash || $.photos.hash);
                                }
                                if (typeof fn == 'function') {
                                    fn();
                                }
            ...
            $.post('?module=photo&action=resolution', photo_ids, function(r) {
                if (r.status == 'ok') {
                    ...
                    $.photos.massPost({
                        ...
                        fullSuccess: function(r) {
                            if (r.status == 'ok') {
                                // clean cache - that photos which has moved to stack
                                var stream_cache = $.photos.photo_stream_cache;
                                for (var i = 0, n = original_photo_ids.length; i < n; ++i) {
                                    stream_cache.deleteById(original_photo_ids[i].value);
                                }
                                if (r.data.alert_msg) {
                                    alert(r.data.alert_msg);
                                }
        ...
        hotkey_manager: (function() {
            function arrowsHandlerDown(e) {
                ...
                if (code == 39) {
                    var next = $.photos.photo_stream_cache.getNext();
                    if (next) {
                        $.photos.goToHash($.photos.getHashByPhotoId(next.id));
                ...
                if (code == 37) {
                    ...
                    if (prev) {
                        $.photos.goToHash($.photos.getHashByPhotoId(prev.id));
                        $.photos.shift_next = true;
                    }
                    arrowsHandlerDown.hold = true;
                }
                ...
                load: function() {
                    ...
                    $.post(
                        ...
                        function (r) {
                            ...
                            var chunk = 10;
                            $.photos.renderPhotoListChunk(target,list_offset,chunk, {string:r.data.string}, function(){
        ...
        onDeleteAlbum: function(album_id) {
            ...
            $('#album-list li[rel='+album_id+']').next('.drag-newposition:first').remove().end().remove();

            // remove from album-list in upload-form
            $('#p-upload-step2 select[name=album_id] option[value='+album_id+']').remove();
        },
        ...
        onUploadDialog: function () {
            ...
            d.delegate('.cancel', 'click', function (e) {
                e.stopPropagation();
                e.preventDefault();
                d.trigger('close');
                return false;
            ...
            if (!hash || !~hash.indexOf('album')) {
                ...
                select.find('optgroup option[value='+album_id+']').attr('selected', true);
            }
        },

        massPost: function(params) {
            var chunk_count = params.chunk_count || 10,
                chunk_photo_ids,
                data = [];
...
$(function () {
     ...
     */
    // when 'shift' held on and there are selected photo forbid defaul browser selecting
    ...
    function photoListItemCheckHandler(e, ext) {
        ...
        if (shiftKey && checked) {   // when hold shift
            ...
            var photo_list = $('#photo-list'),
                started = photo_list.data('last_checked') || photo_list.find('li:first').addClass('selected');

            // try find started before current
            var found = self.prevAll('.selected[data-photo-id='+started.attr('data-photo-id')+']');
            if (found.length) {
                var started = started.get(0),
                    li = self.prev();
                ...
                found = self.nextAll('.selected[data-photo-id='+started.attr('data-photo-id')+']');
                if (found.length) {
                    var started = started.get(0),
                        li = self.next();
                    while (li.length && started != li.get(0)) {
            ...
            if (!photo_list.data('last_checked') && !found.length) {
                started.removeClass('selected');
            }
        } else {
            if (checked) {
                $("#photo-list").data('last_checked', self);
            }
        }
        $('#share-menu-block, #organize-menu-block').trigger('recount');
    }
    
    $("#photo-list li input").live('click', photoListItemCheckHandler);

    // when select photo by hand (see below). Firefox version
    // In Firefox shift+click on label don't delegate to coresponding input
    if ($.browser.mozilla) {
        $("#photo-list li label").live('click', function(e) {
            if (!e.shiftKey) {
                return;
            }
            var item = $(e.target),
            ...
            input = $('#' + item.attr('for'));
            input.trigger('click', {shiftKey: e.shiftKey, checked: !input.attr('checked')});
        });
    }
