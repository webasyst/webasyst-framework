...
class siteSystemSettingsGenerateDkimController extends waJsonController
{
    public function execute()
    {
        ...
        $one_string_key = siteHelper::getOneStringKey($dkim_pub_key);

        $email = trim(waRequest::post('email'));
        $e = explode('@', $email);

        $selector = siteHelper::getDkimSelector($email);
