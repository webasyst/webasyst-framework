(function(a){function c(b){var c=a(window),d=c.scrollTop(),e=c.scrollLeft()+c.width(),h=c.scrollTop()+c.height(),c=c.scrollLeft(),j=b.element.offset();j.right=j.left+b.element.outerWidth();j.bottom=j.top+b.element.outerHeight();return d<=j.top&&c<=j.left&&e>=j.right&&h>=j.bottom}function b(b,c){return b.clone().empty().addClass(c.fixed_class||"sticky-fixed").css(a.extend({position:"fixed",display:"none"},c.fixed_css||{})).removeAttr("id").insertAfter(b)}a.fn.sticky=function(g){function f(){d.closest("body").length?
