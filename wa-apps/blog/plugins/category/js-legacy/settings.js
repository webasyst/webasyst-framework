if (!$.wa_blog_plugins_category) {
    $.wa_blog_plugins_category = {
        options: {
            loader: '<i class="b-loading icon16 loading"></i>'
        },
        counter: 0,
        ajaxInit: function () {
            //this.ajaxPurge();
            ...
            $container.find('.b-category-add').on('click.plugins_category', function (eventObject) {
                return self.addHandler.apply(self, [this, eventObject]);
            });
            // inline edit
            $container.find('.b-category-edit').on('click.plugins_category', function (eventObject) {
                return self.editHandler.apply(self, [this, eventObject]);
            });
            $container.find('.b-category-delete').on('click.plugins_category', function (eventObject) {
                return self.deleteHandler.apply(self, [this, eventObject]);
            });
            $container.find('.b-plugins-category-icon').on('change.plugins_category', function (eventObject) {
                return self.iconHandler.apply(self, [this, eventObject]);
            });
            $('#plugin-category-form').on('submit.plugins_category', function (eventObject) {
