...
class blogImportPluginWordpressTransport extends blogImportPluginTransport
{
        ...
        if (isset($filters[$filter])) {
            foreach ($filters[$filter] as $method) {
                if (is_callable($callback =array($this,$method))) {
                    ...
                    $this->log(__METHOD__." {$method} is uncallable",self::LOG_WARNING);
                }
            }
        }
        return $content;
    ...
    private static function wpautop($pee, $br = 1)
    {
     ...
     */
    private static function convert_chars($content, $deprecated = '')
    {
