...
class dictionariesItemsModel extends waModel
{
    ...
    {
        $sql = "SELECT * FROM {$this->table} WHERE dictionary_id=i:list_id ORDER BY sort";
    ...
    {
        $sql = "UPDATE {$this->table} SET sort=sort+1 WHERE dictionary_id=i:list_id AND sort >= i:sort";
