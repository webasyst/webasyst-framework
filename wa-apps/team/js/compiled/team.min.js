...
f.off("click.map",".map-link").on("click.map",".map-link",function(){var p=c(this).parent().data("multifield-index");void 0!==p&&g.geocodeAddress(g.fieldEditors.address.fieldValue,p)}));if("edit"!=b||!f.hasClass("edit-mode"))if("view"!=b||!f.hasClass("view-mode")){c(this).trigger("before_switch_mode",[b,this]);f.find(".buttons").remove();for(var h=[],m=0;m<this.fieldsOrder.length;m+=1){var n=this.fieldsOrder[m];h.push(n);try{var l=this.fieldEditors[n].setMode(b);c(this).trigger("set_mode",[{mode:b,
el:f,field_id:n,field:this.fieldEditors[n]}]);d&&f.append(l)}catch(p){console.log("Error initializing field",n,p)}}"edit"==b?(f.addClass("edit-mode"),f.removeClass("view-mode"),f.find(".subname").wrapAll('<div class="subname-wrapper custom-mt-0"></div>'),f.find(".jobtitle-company").wrapAll('<div class="jobtitle-company-wrapper"></div>'),d=this.inplaceEditorButtons(h,function(p){if("undefined"!=typeof p&&!p)return!1;if("undefined"!=typeof g.justCreated&&g.justCreated)throw p=c("#all-users-sidebar-link .count"),
p.text(1+parseInt(p.text())),Error("!!! Not implemented because never used. Redirect to contact_id="+g.contact_id);g.dialog.hide()}),null===g.contact_id&&d.find(".cancel").remove(),g.dialog.$body.find(".dialog-footer").append(d)):(f.addClass("view-mode"),f.removeClass("edit-mode"),f.find(".subname-wrapper").length&&f.find(".subname").unwrap(),f.find(".jobtitle-company-wrapper").length&&f.find(".jobtitle-company").unwrap());c(this).trigger("after_switch_mode",[b,this])}},saveFields:function(b,d){function f(x,
y,C){if(C)for(var A=0;A<C.length;A+=1){if(x[C[A]]&&y[C[A]]&&x[C[A]]!=y[C[A]])return!1}else{for(A in x)if(x.hasOwnProperty(A)&&x[A]&&y[A]&&x[A]!=y[A])return!1;for(A in y)if(y.hasOwnProperty(A)&&x[A]&&y[A]&&x[A]!=y[A])return!1}return!0}function g(x){x=void 0===x?!0:x;c.post(l.getSaveUrl(),{data:JSON.stringify(n),type:l.contactType,id:null!=l.contact_id?l.contact_id:0},function(y){if("ok"!==y.status){if(y.errors.length){y=y.errors["0"];c(".buttons .loading").hide();c("#validation-notice").text(y.text);
return}throw new Exception("AJAX error:",y);}c("#validation-notice").text("");var C=l.fieldsValues||{};y=y.data;var A=null;y.data&&y.data.top&&(A=y.data.top);y.data.top=void 0;delete y.data.top;null!=l.contact_id&&l.initFieldEditors(y.data);var D=!1,B;for(B in l.fieldEditors)if("undefined"!=typeof y.errors[B]){if(l.fieldEditors[B].showValidationErrors(y.errors[B]),!D)for(D=l.fieldEditors[B].domElement;!D.is(":visible");)D=D.parent()}else"edit"==l.fieldEditors[B].currentMode&&l.fieldEditors[B].showValidationErrors(null);
D||(l.contact_id&&y.data.reload?window.profileTab?window.profileTab.rootWindow.location.reload():window.location.reload():(null===l.contact_id&&(l.justCreated=!0),l.contact_id=y.data.id,!D&&x&&h(C,y),d(!D),D||(c(e).trigger("contact_saved",y.data),A&&c(e).trigger("top_fields_updated",{data:A}),c.team.content.reload(!0))))},"json")}function h(x,y){var C=c.storage.get("contacts/last_geocoding")||0;if(3600<(new Date).getTime()-C&&(c.storage.del("contacts/last_geocoding"),y=y.data.address,!c.isEmptyObject(y))){var A=
[],D=[];for(C=0;C<y.length;C+=1){var B=!0;n.address[C]&&(B=!f(y[C].data,(x.address[C]||{}).data||{},["city","country","region","street","zip"]));B&&(A.push(l.sendGeocodeRequest(y[C].for_map)),D.push(C))}if(A.length){var G=function(z,F){if("OK"===z.status){var k=z.lng||"";n.address[F].value.lat=z.lat||"";n.address[F].value.lng=k}else"OVER_QUERY_LIMIT"===z.status&&c.storage.set("contacts/last_geocoding",(new Date).getTime()/1E3)};c.when.apply(c,A).then(function(){if(1>=A.length&&"success"===arguments[1])G(arguments[0],
D[0]);else for(var z=0;z<arguments.length;z+=1)"success"===arguments[z][1]&&G(arguments[z][0],D[z]);g(!1)})}}}if(!b){b=[];for(var m in this.fields)this.fields.hasOwnProperty(m)&&b.push(m)}var n={},l=this;m=!1;for(var p=0;p<b.length;p++){var t=b[p],v=this.fieldEditors[t].validate();if(v){if(!m)for(m=this.fieldEditors[t].domElement;!m.is(":visible");)m=m.parent();this.fieldEditors[t].showValidationErrors(v)}else this.fieldEditors[t].showValidationErrors(null);n[t]=this.fieldEditors[t].getValue()}m?
d(!1):g(!0)},createExtSelect:function(b,d){var f="",g=!0,h;for(h in b){var m="";if(b[h]===d||h===d)m=' selected="selected"',g=!1;var n=this.htmlentities(b[h]);f+='<option value="'+("undefined"===typeof b.length?h:n)+'"'+m+">"+n+"</option>"}if(g){f+='<option value="%custom" selected="selected">'+$_("other")+"...</option>";var l='<input type="text" class="shorter small ext width-100-mobile custom-mt-12-mobile">'}else f+='<option value="%custom">'+$_("other")+"...</option>",l='<input type="text" class="shorter small empty ext width-100-mobile custom-mt-12-mobile">';
f=c('<div class="wa-select small width-100-mobile custom-mr-12 custom-mt-12-mobile"><select class="ext">'+f+'</select></div><span class="width-100-mobile custom-mr-12 custom-mr-0-mobile">'+l+"</span>");var p=f.children("select");l=f.find("input");l.val(d);"%custom"!==p.val()&&l.hide();d=$_("which?");var t=function(){l.val()||l.hasClass("empty")||(l.val(d),l.addClass("empty"))};l.blur(t);l.focus(function(){l.hasClass("empty")&&(l.val(""),l.removeClass("empty"))});p.change(function(){var v=p.val();
"%custom"===v?(l.hasClass("empty")&&l.val(d),l.show()):(l.hide(),l.addClass("empty"),l.val(v||""));t()});l[0].getExtValue=function(){return"%custom"===p.val()&&l.hasClass("empty")?"":l.val()};l[0].setExtValue=function(v){b[v]?p.val(v):p.val("%custom");l.val(v)};return f},inplaceEditorButtons:function(b,d){var f=c('<div class="field buttons"><div class="value"><p class="state-error-hint custom-mt-8" id="validation-notice"></p></div></div>'),g=this,h=function(){c(".buttons .loading").show();g.saveFields(b,
function(l){c(".buttons .loading").hide();d(l)});return!1};c("#tc-contact").off("keyup.contact_save").on("keyup.contact_save",'#contact-info-block.edit-mode input[type="text"]',function(l){13!=l.keyCode||c(l.currentTarget).data("autocomplete")&&c(l.currentTarget).data("contact_id")||h()});var m=c('<input type="submit" class="button" value="'+$_("Save")+'" />').click(h);g=this;var n=c('<a href="javascript:void(0)" class="cancel button light-gray js-close-dialog">'+$_("Cancel")+"</a>");n.on("click",
function(l){l.preventDefault();c("#validation-notice").text("");c(".buttons .loading").hide();g.fieldEditors.name.showValidationErrors(null)});f.children(".value").append(m).append(n).append(c('<i class="fas fa-spinner fa-spin loading custom-ml-16 wa-animation-spin speed-1000" style="display: none;"></i>'));return f},destroy:function(){c(this.el).html("")},dialogInstance:function(b){this.dialog=b},wrapper:function(b,d,f){f=c('<div class="field'+("undefined"!=typeof f&&f?" "+f:"")+'"></div>');"undefined"!=
typeof d&&d&&(c(this).on("set_mode",function(g,h){"Checkbox"===h.field.fieldData.type?h.field.domElement.find(".name").toggleClass("for-checkbox","edit"==h.mode):h.field.domElement.find(".name").toggleClass("for-input","edit"==h.mode)}),f.append('<div class="name'+(c(this.el).closest(".dialog-content").length?" for-input":"")+'">'+c.wa.encodeHTML(d)+"</div>"));if("object"!=typeof b||!(b instanceof jQuery)||0>=b.find("div.value").size())b=c('<div class="value custom-mb-12"></div>').append(b);f.append(b);
return f},htmlentities:function(b){var d=document.createElement("div");b=document.createTextNode(b);d.appendChild(b);return d.innerHTML},geocodeAddress:function(b,d){var f=this;b[d].data.lat&&b[d].data.lng||this.sendGeocodeRequest(b[d].for_map||b[d].value,function(g){b[d].data.lat=g.lat;b[d].data.lng=g.lng;c.post(f.getSaveGeocoordsUrl(),{id:f.contact_id,lat:g.lat,lng:g.lng,sort:d})})},sendGeocodeRequest:function(b,d){var f=[];"string"===typeof b?f.push(b):b.with_street&&b.without_street&&b.with_street===
