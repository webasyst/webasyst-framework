...
this])}},saveFields:function(b,d){function f(y,x,C){if(C)for(var A=0;A<C.length;A+=1){if(y[C[A]]&&x[C[A]]&&y[C[A]]!=x[C[A]])return!1}else{for(A in y)if(y.hasOwnProperty(A)&&y[A]&&x[A]&&y[A]!=x[A])return!1;for(A in x)if(x.hasOwnProperty(A)&&y[A]&&x[A]&&y[A]!=x[A])return!1}return!0}function g(y){y=void 0===y?!0:y;c.post(l.getSaveUrl(),{data:JSON.stringify(n),type:l.contactType,id:null!=l.contact_id?l.contact_id:0},function(x){if("ok"!==x.status){if(x.errors.length){x=x.errors["0"];c(".buttons .loading").hide();
c("#validation-notice").text(x.text);return}throw new Exception("AJAX error:",x);}c("#validation-notice").text("");var C=l.fieldsValues||{};x=x.data;var A=null;x.data&&x.data.top&&(A=x.data.top);x.data.top=void 0;delete x.data.top;null!=l.contact_id&&l.initFieldEditors(x.data);var D=!1,B;for(B in l.fieldEditors)if("undefined"!=typeof x.errors[B]){if(l.fieldEditors[B].showValidationErrors(x.errors[B]),!D)for(D=l.fieldEditors[B].domElement;!D.is(":visible");)D=D.parent()}else"edit"==l.fieldEditors[B].currentMode&&
l.fieldEditors[B].showValidationErrors(null);D||(l.contact_id&&x.data.reload?window.profileTab?window.profileTab.rootWindow.location.reload():window.location.reload():(null===l.contact_id&&(l.justCreated=!0),l.contact_id=x.data.id,!D&&y&&h(C,x),d(!D),D||(c(e).trigger("contact_saved",x.data),A&&c(e).trigger("top_fields_updated",{data:A}),c.team.content.reload(!0))))},"json")}function h(y,x){var C=c.storage.get("contacts/last_geocoding")||0;if(3600<(new Date).getTime()-C&&(c.storage.del("contacts/last_geocoding"),
x=x.data.address,!c.isEmptyObject(x))){var A=[],D=[];for(C=0;C<x.length;C+=1){var B=!0;n.address[C]&&(B=!f(x[C].data,(y.address[C]||{}).data||{},["city","country","region","street","zip"]));B&&(A.push(l.sendGeocodeRequest(x[C].for_map)),D.push(C))}if(A.length){var G=function(z,F){if("OK"===z.status){var k=z.lng||"";n.address[F].value.lat=z.lat||"";n.address[F].value.lng=k}else"OVER_QUERY_LIMIT"===z.status&&c.storage.set("contacts/last_geocoding",(new Date).getTime()/1E3)};c.when.apply(c,A).then(function(){if(1>=
A.length&&"success"===arguments[1])G(arguments[0],D[0]);else for(var z=0;z<arguments.length;z+=1)"success"===arguments[z][1]&&G(arguments[z][0],D[z]);g(!1)})}}}if(!b){b=[];for(var m in this.fields)this.fields.hasOwnProperty(m)&&b.push(m)}var n={},l=this;m=!1;for(var r=0;r<b.length;r++){var u=b[r],v=this.fieldEditors[u].validate();if(v){if(!m)for(m=this.fieldEditors[u].domElement;!m.is(":visible");)m=m.parent();this.fieldEditors[u].showValidationErrors(v)}else this.fieldEditors[u].showValidationErrors(null);
n[u]=this.fieldEditors[u].getValue()}m?d(!1):g(!0)},createExtSelect:function(b,d){var f="",g=!0,h;for(h in b){var m="";if(b[h]===d||h===d)m=' selected="selected"',g=!1;var n=this.htmlentities(b[h]);f+='<option value="'+("undefined"===typeof b.length?h:n)+'"'+m+">"+n+"</option>"}if(g){f+='<option value="%custom" selected="selected">'+$_("other")+"...</option>";var l='<input type="text" class="shorter small ext width-100-mobile custom-mt-12-mobile">'}else f+='<option value="%custom">'+$_("other")+"...</option>",
l='<input type="text" class="shorter small empty ext width-100-mobile custom-mt-12-mobile">';f=c('<div class="wa-select small width-100-mobile custom-mr-12 custom-mt-12-mobile"><select class="ext">'+f+'</select></div><span class="width-100-mobile custom-mr-12 custom-mr-0-mobile">'+l+"</span>");var r=f.children("select");l=f.find("input");l.val(d);"%custom"!==r.val()&&l.hide();d=$_("which?");var u=function(){l.val()||l.hasClass("empty")||(l.val(d),l.addClass("empty"))};l.blur(u);l.focus(function(){l.hasClass("empty")&&
(l.val(""),l.removeClass("empty"))});r.change(function(){var v=r.val();"%custom"===v?(l.hasClass("empty")&&l.val(d),l.show()):(l.hide(),l.addClass("empty"),l.val(v||""));u()});l[0].getExtValue=function(){return"%custom"===r.val()&&l.hasClass("empty")?"":l.val()};l[0].setExtValue=function(v){b[v]?r.val(v):r.val("%custom");l.val(v)};return f},inplaceEditorButtons:function(b,d){var f=c('<div class="field buttons"><div class="value"><p class="state-error-hint custom-mt-8" id="validation-notice"></p></div></div>'),
...
c.post(l.attr("action"),l.serialize(),function(v){l.find(".loading").hide();if("ok"===v.status){c("#c-login-block").show();r.val(u);l.find(".c-login").text(u);l.find(".c-one-tab").show();l.find(".c-two-tab").hide();if(!d)try{window.profileTab.reload()}catch(y){}v=window.location.href.replace(d,u);d=u;window.location.assign(v)}else"fail"===v.status&&l.find('input[type="submit"]').parent().prepend(c('<p class="state-error-hint custom-mt-4 custom-mb-8">'+v.errors.join("\n<br>\n")+"</p>"))},"json");return!1});
l.find(".c-tab-toggle").click(function(u){u.preventDefault();l.find(".c-one-tab,.c-two-tab").toggle();r.is(":visible")&&r.focus();return!1})})();(function(){var l=c("#c-password-form"),r=l.find(".c-password-input"),u=l.find(".c-confirm-password-input");l.submit(function(){l.find("input.state-error").removeClass("error");l.find(".state-error-hint").remove();if(r.val()!==u.val())return r.addClass("error"),u.after().after('<p class="state-error-hint custom-mt-4">'+n["Passwords do not match."]+"</p>"),
!1;l.find(".loading").show();c.post(l.attr("action"),l.serialize(),function(v){l.find(".loading").hide();"ok"===v.status?(f=!0,r.val(""),u.val(""),l.find(".c-one-tab").show(),l.find(".c-two-tab").hide(),c("#c-password-block").show()):"fail"===v.status&&u.after('<p class="state-error-hint custom-mt-4">'+v.errors.join("<br />")+"</p>")},"json");return!1});l.find(".c-tab-toggle").click(function(v){v.preventDefault();l.find(".c-one-tab,.c-two-tab").toggle();v=l.find(".c-password-input");v.is(":visible")&&
v.focus();return!1})})();(function(){var l=c("#tc-api-tokens-filed"),r=l.find(".js-api-tokens-list"),u=!1;l.on("click",".js-remove-api-token",function(v){v.preventDefault();var y=c(this).parents(".js-token-item"),x=y.find(".icon16");v=y.data("token");var C={action:"remove",token_id:v};!u&&v&&confirm(n.remove_ask)&&(u=!0,x.removeClass("no").addClass("loading"),c.post(m,C,function(A){A.status&&"ok"===A.status?(y.remove(),0===r.find(".js-token-item").length&&l.remove()):(u=!1,x.removeClass("loading").addClass("no"))}).always(function(){u=
!1;x.removeClass("loading").addClass("no")}))})})();a.is_own_profile||b(a.wa_url,a.wa_framework_version);a.is_own_profile&&e();(function(){c(".js-webasyst-id-unbind-auth").on("click",function(l){l.preventDefault();c(".js-webasyst-id-unbind-auth").trigger("wa_waid_unbind_auth",{id:g})})})();(function(){c(".js-webasyst-id-help-link").on("click",function(l){l.preventDefault();c("body").trigger("wa_waid_help_link")})})();(function(l,r,u){function v(){y(!0)&&x()}function y(B){c("#c-access-rights-hint-warning").hide();
c("#c-access-rights-hint-customize").hide();var G=C.val();void 0===G&&(G="1");switch(G){case "remove":c("#c-credentials-block").hide();c("#c-login-block").hide();c("#c-password-block").hide();if(u){c("#c-access-rights-by-app").hide();c(".c-shown-on-access").hide();break}C.val(D||"0");c("#c-access-rights-hint-warning").show();return!1;case "0":if(d||f)if(d){var z=c("#c-access-rights-by-app");z.show();B&&z.find(".t-access-status").removeClass("type-no type-limited type-full").addClass("type-no");c(".c-shown-on-access").show();
c("#c-login-block").show();c("#c-password-block").show();break}else return c("#c-login-block").show().find(".cancel").one("click.access",function(){C.val(D);v()}).end().find(".c-tab-toggle:first").click(),c("#c-password-block").show(),!1;else return c("#c-credentials-block").show().find(".c-login-input").focus().end().find(".cancel").one("click.access",function(){C.val(D);v()}),!1;case "1":if(d||f)if(d){c("#c-access-rights-by-app").hide();c(".c-shown-on-access").show();c("#c-login-block").show();
c("#c-password-block").show();break}else return c("#c-login-block").show().find(".cancel").one("click.access",function(){C.val(D);v()}).end().find(".c-tab-toggle:first").click(),c("#c-password-block").show(),!1;else return c("#c-credentials-block").show().find(".c-login-input").focus().end().find(".cancel").one("click.access",function(){C.val(D);v()}),!1;default:return!1}l&&(c("#c-login-block").show(),c("#c-password-block").show());D=G;return!0}function x(){function B(z,F,k){return c.post(h+"?module=accessSave&action=rights&id="+
