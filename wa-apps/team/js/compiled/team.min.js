...
function E(x){return b.post(h+"?module=accessSave&action=singleAppMode&id="+g,{app_id:x},"json")}function B(x,F,k){return b.post(h+"?module=accessSave&action=rights&id="+g,{app_id:x,name:F,value:k},"json")}var C=b("#c-access-rights-toggle"),A=b("#access-rights-toggle-confirm"),G=C.val();q?b(".c-shown-on-access").hide():b(".c-shown-on-access").show();y();A.on("click",".cancel",function(){C.val(G);A.hide()});A.on("click",".button",function(){A.hide();v()});C.change(function(){if(d){b("#c-access-rights-hint-warning").hide();
b("#c-access-rights-hint-customize").hide();var x=C.val();void 0===x&&(x="1");x===G?A.hide():A.show()}else y(!0)})})(a.is_own_profile,a.contact_no_access,a.contact_groups_no_access);new UserAccessTable({$wrapper:b(".c-access-rights-wrapper"),contact_id:g,is_frame:!0})};window.UserAccessTable=function(a){var e=a.$wrapper,c=a.contact_id,d=a.is_frame||!1;e.on("click",".t-access-status",function(g){g.preventDefault();g=b(this);showAccessDialog(g,g.data("app-id"),c,!0,d)});var f;b(document).on("team_access_level_changed",
f=function(g,h){b.contains(document.body,e[0])?e.find('.t-access-status[data-app-id="'+h.app_id+'"]').removeClass("type-no type-limited type-full type-"+h.prev_level).addClass("type-"+h.new_level):b(document).off("team_access_level_changed",f)})};window.showAccessDialog=function(a,e,c,d,f){a.trigger("close");b.post(b.team.app_url+"?module=access&action=dialog",{user_id:c,app_id:e},function(g){g={html:g};f&&d&&(g.setPosition=function(h){var m=b(window).width();return{top:a.offset().top,left:parseInt((m-
h.width)/2)}});b.waDialog(g)})}})(jQuery);var GroupPage=function(b){GroupPage=function(a){this.$wrapper=a.$wrapper;this.group_id=a.group_id;this.map_adapter=a.map_adapter;this.latitude=a.latitude;this.longitude=a.longitude;this.address=a.address;this.can_manage=a.can_manage;this.initClass()};GroupPage.prototype.initClass=function(){this.bindEvents();this.can_manage&&(this.initEditableName(),this.initEditableDescription());this.initInfoBlock();this.initMap()};GroupPage.prototype.bindEvents=function(){var a=this;a.$wrapper.on("click",".js-open-map-link",
