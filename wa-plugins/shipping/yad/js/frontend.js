/**
 * @typedef {object} shippingYadPickup
 * @property {string}  id
 * @property {float} lat
 * @property {float} lng
...
/**
 * @typedef {object} shippingYadCourier
 * @property {Array.string} intervals
 * @property {string} placeholder
 * @property {number} offset
 */
/**
 * @typedef {object} shippingYadGeocode
...
/**
 * @typedef {object} shippingYadResponseData
 * @property {Array.<shippingYadResponseService>}  services
 * @property {Array.<shippingYadGeocode>}  options
 */

/**
 * @typedef {object} shippingYadResponseService
 * @property {string} type
 * @property {Array.<shippingYadPickup>}  pickup
 * @property {Array.<shippingYadCourier>}  courier
 */


/**
 * @typedef {object} shippingYadResponseError
 * @property {string} error
 * @property {object} data
 */

/**
 * @typedef {object} shippingYadResponse
 * @property {string} status
 * @property {shippingYadResponseData} data
 * @property {shippingYadResponseError} error
 */

/**
 * @typedef {object} shippingYadId
 * @property {string} map
 ...
 *
 * @param {string} key
 * @param {shippingYadId} id
 * @param {string} url
 * @returns {ShippingYad}
 * @constructor
 */
function ShippingYad(key, id, url) {
    this.options = {
    ...
    this.loader = {
        ...
        init: function (control) {
            if (control) {
                this.loader_container = control.find('.yad-loading-section');
            }
        },
        show: function () {
    ...
    this.rate = {
        ...
        load: function () {
            ...
            $.ajax({
                ...
                /**
                 *
                 * @param {shippingYadResponse} response
                 */
                "success": function (response) {
                    if (response.status === 'ok') {
                        if (response.data) {
                            ...
                            self.filter_select.off('change.yad').on('change.yad', function () {
                                /**
        ...
        init: function (form, select) {
            ...
            /**
             * @var {ShippingYad.rate} self
             */

            this.rate_select.off('change.yad').on('change.yad', function () {
                self.change(this);
            });
            this.geo_id_select.off('change.yad').on('change.yad', function () {
                self.suggest(this);
            });


            this.filter_select.off('change.yad');
    ...
    this.pickup = {
        ...
        /**
         * @property {Array.<shippingYadPickup>} pickups
         */
        pickups: {},
        /**
         *
        ...
        init: function (form) {
            if (form) {
                this.pickup_container = form.find('.yad-map-section');
                this.pickup_container.hide();

                this.radio = form.find(':input[name$="\.pickup_id\]"]:first').parents('.js-yad-variant:first');
                this.radio_id = this.radio.find(':input').attr('id');
                this.radio.hide();

                this.pickup_section = form.find('.yad-pickup-section');
                this.pickup_list = form.find('.yad-aside:first ul');


                var self = this;
                this.pickup_list.off('change.yad').on('change.yad', ':input[name$="\.pickup_id\]"]', function (event) {
                    var input = $(this);
                    self.pickup_list.find('input.js-yad-button:visible').hide();
                ...
                /* map click handler */
                $('html').off('click.yad_' + self.radio_id).on('click.yad_' + self.radio_id, '.js-yad-button-' + self.radio_id, function () {
                    var button = $(this);
                ...
                this.pickup_container.off('click.yad').on('click.yad', '.js-yad-button', function () {
                    var button = $(this);
                    var id = button.data('id');
                    instance.rate.select('pickup', id);
                    self.show(id);
                ...
                this.pickup_section.off('click.yad').on('click.yad', '.js-yad-change', function () {
                    self.change();
                    return false;
                })
            }
        ...
        reset: function () {
            if (this.pickup_container) {
                this.pickup_container.find('.js-yad-variant:not(:first)').remove();
            }
            this.map.reset();
        },
        hide: function () {
        ...
        /**
         *
         * @param {Array.<shippingYadPickup>} pickups
         * @param {Number} pickup_id
         */
        load: function (pickups, pickup_id) {

            /**
             * @this ShippingYad.pickup
             */
        ...
        /**
         *
         * @param {shippingYadPickup} pickup
        ...
        add: function (pickup, checked) {
            pickup.input_id = this.radio_id + pickup.id;
            ...
            return {
                ...
                "properties": {
                    ...
                    (pickup.schedule_html ? ('<div class="line yad-list">' + pickup.schedule_html + '</div>') : ''),
                    "balloonContentHeader": pickup.title,
                    "balloonContentFooter": '<div class="line actions">' +
                    '<div class="t-layout">' +
                    '<div class="t-column middle">' +
                    '<input class="js-yad-button js-yad-button-' + this.radio_id + '" type="button" value="Выбрать" data-id="' + pickup.id + '">' +
                    '</div>' +
                    '</div>' +
                    '</div>',
        ...
        show: function (id) {
            ...
            /**
             * @var {shippingYadPickup} pickup
             */
            if (pickup) {
                this.pickup_section.find('.yad-pickup-header').text(pickup.title);

                this.pickup_section.find('.line.js-yad-address').text((pickup.description !== pickup.title) ? pickup.description.replace(/[\r\n]+/g, '<br/>') : '');
                var payment = (typeof pickup.payment === 'object') ? $.map(pickup.payment, function (p) {
                    return p;
                }).join(", ") : pickup.payment;
                this.pickup_section.find('.line.js-yad-payment').html(payment);
                this.pickup_section.find('.line.js-yad-schedule').html(pickup.schedule_html);
    ...
    if (this.form.length && this.container) {
        ...
        input.on("change.yad", 'select[name="rate_id[' + this.key + ']"]', function () {
            setTimeout(function () {
