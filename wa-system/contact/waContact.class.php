...
class waContact implements ArrayAccess
{
     ...
     *
     * By default, hash is generated using PHP function password_hash() with PASSWORD_DEFAULT (bcrypt).
     * The password is pre-hashed with SHA-256 so that bcrypt's 72-byte input limit does not truncate
     * arbitrarily long passwords.
     * If configuration file wa-config/SystemConfig.class.php contains information about user-defined
     * function wa_password_hash(), then that function is used for generating password hash instead.
     *
    ...
    public static function getPasswordHash($password)
    {
        if (function_exists('wa_password_hash')) {
            ...
            return password_hash(hash('sha256', $password).$password, PASSWORD_DEFAULT);
        }
    }
     ...
     *
     * Supports both bcrypt hashes (generated by password_hash() / getPasswordHash())
     * and legacy md5 hashes for backwards compatibility with older framework versions.
     *
     * If configuration file wa-config/SystemConfig.class.php contains a user-defined
     * function wa_password_verify(), then that function is used for hash verification.
     *
    ...
    public static function verifyPasswordHash($password, $hash)
    {
        ...
        // Detect hash format: bcrypt hashes start with $2y$, $2a$, or $2b$
        if (strlen($hash) !== 32 || substr($hash, 0, 2) === '$2') {
            return password_verify(hash('sha256', $password).$password, $hash);
        }

        // Legacy md5 hash — compare deterministically
